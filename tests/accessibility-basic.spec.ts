...
test.describe('WCAG 2.1 AA Accessibility Compliance', () => {
  ...
  test('keyboard navigation works', async ({ page }) => {
    ...
    if (await firstButton.isVisible()) {
      ...
      console.log('First button successfully focused');
    }
    ...
    console.log(`After tabbing: focused on ${activeElement.tagName}${activeElement.id ? '#' + activeElement.id : ''}`);
  });
  ...
  test('forms have proper labeling', async ({ page }) => {
    ...
    if (inputCount > 0) {
      for (let i = 0; i < inputCount; i++) {
        ...
        console.log(`Input ${i + 1}: ID="${inputId}", hasLabel=${hasLabel}, ariaLabel="${ariaLabel}"`);
  ...
  test('images have alt text', async ({ page }) => {
    ...
    if (imageCount > 0) {
      for (let i = 0; i < imageCount; i++) {
        ...
        console.log(`Image ${i + 1}: alt="${alt}", role="${role}"`);
      }
    } else {
  ...
  test('color contrast - focus indicators are visible', async ({ page }) => {
  ...
  test('responsive design - no horizontal scroll at narrow widths', async ({ page }) => {
    ...
    console.log(`Horizontal scroll at 320px: ${hasHorizontalScroll}`);
    // Note: We'll log this but not fail the test as some layouts may legitimately need horizontal scroll
    if (hasHorizontalScroll) {
      console.warn('Horizontal scrolling detected at 320px width - check mobile layout');
    }
  });
  ...
  test('touch targets are adequate size on mobile', async ({ page }) => {
    ...
    console.log(`Small touch targets found: ${smallTargets}/${Math.min(elementCount, 10)}`);
    
    // Allow some tolerance but warn if many targets are too small
    if (smallTargets > elementCount * 0.5) {
      console.warn('Many touch targets are smaller than 44x44px - consider increasing sizes');
    }
  });
});